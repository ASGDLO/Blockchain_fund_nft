...
describe('Multicall v2 encoder', function () {
    ...
    it('Properly encodes', function () {
        var calldata = constants_1.ethersAbi.encodeFunctionData('tryAggregate', [true, calls.map(function (calldata) { return [address, calldata]; })]);
        var manual = (0, encoder_1.encodeTryAggregate)(true, calls.map(function (calldata) { return [address, calldata]; }));
    ...
    it('bench ethers', function () {
        ...
        (0, benchmark_1.formatBench)((0, benchmark_1.bench)(function () {
            constants_1.ethersAbi.encodeFunctionData('tryAggregate', [true, callsLong.map(function (calldata) { return [address, calldata]; })]);
        }));
    });
    it('bench manual', function () {
        var callsLong = __spreadArray([], Array(20), true).flatMap(function () { return calls; });
        (0, benchmark_1.formatBench)((0, benchmark_1.bench)(function () {
            (0, encoder_1.encodeTryAggregate)(true, callsLong.map(function (calldata) { return [address, calldata]; }));
        }));
    });
});
