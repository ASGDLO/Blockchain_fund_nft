...
import { deployMockToken, renderDAppHook, SECOND_TEST_CHAIN_ID, setupTestingConfig } from '../testing';
...
describe('useTokenBalance', () => {
    ...
    it('returns balance for explicitly mainnet', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useTokenBalance(token1.address, receiver, { chainId: Mainnet.chainId }), { config });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
        expect(result.current).to.eq(100);
    });
    it('returns balance for explicitly another chain', async () => {
        const { result, waitForCurrent } = await renderDAppHook(() => useTokenBalance(token2.address, receiver, { chainId: SECOND_TEST_CHAIN_ID }), { config });
        await waitForCurrent((val) => val !== undefined);
        expect(result.error).to.be.undefined;
