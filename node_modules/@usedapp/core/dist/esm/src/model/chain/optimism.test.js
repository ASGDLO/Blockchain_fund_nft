...
describe('Optimism Chain', () => {
    ...
    it('getExplorerAddressLink', () => {
        expect(Optimism.getExplorerAddressLink(TEST_ADDRESS)).to.eq(`https://optimistic.etherscan.io/address/${TEST_ADDRESS}`);
        expect(OptimismKovan.getExplorerAddressLink(TEST_ADDRESS)).to.eq(`https://kovan-optimistic.etherscan.io/address/${TEST_ADDRESS}`);
    });
    it('getExplorerTransactionLink', () => {
        expect(Optimism.getExplorerTransactionLink(TEST_TX)).to.eq(`https://optimistic.etherscan.io/tx/${TEST_TX}`);
        expect(OptimismKovan.getExplorerTransactionLink(TEST_TX)).to.eq(`https://kovan-optimistic.etherscan.io/tx/${TEST_TX}`);
    });
});
