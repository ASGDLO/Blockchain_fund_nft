...
var AbiGenerator = /** @class */ (function () {
    ...
    AbiGenerator.prototype.watchForChanges = function () {
        ...
        var fsWait = false;
        fs_extra_1.default.watch(this.getAbiFileFullPathLocation(), function (_event, filename) {
            if (filename) {
                if (fsWait)
                    return;
                setTimeout(function () {
        ...
        if (abiItem.inputs) {
            for (var i = 0; i < abiItem.inputs.length; i++) {
                ...
                if (abiItem.inputs[i].type.includes(abi_properties_1.SolidityType.tuple)) {
                    ...
                    input += inputName + ": " + helpers_2.default.getSolidityInputTsType(abiItem.inputs[i].type, this._context.provider);
                }
            }
        }
    ...
    AbiGenerator.prototype.buildTupleParametersInterface = function (name, abiInput) {
        ...
        for (var i = 0; i < abiInput.components.length; i++) {
            properties += abiInput.components[i].name + ": " + helpers_2.default.getSolidityInputTsType(abiInput.components[i].type, this._context.provider) + ";";
        }
    ...
    AbiGenerator.prototype.buildTupleResponseInterface = function (abiOutput) {
        var interfaceName = helpers_2.default.buildResponseInterfaceName(abiOutput);
        var properties = '';
        for (var i = 0; i < abiOutput.components.length; i++) {
            ...
            if (abiOutput.components[i].components) {
                var deepInterfaceName = helpers_2.default.buildResponseInterfaceName(abiOutput.components[i]);
                var deepProperties = '';
        ...
        if (abiItem.outputs && abiItem.outputs.length > 0) {
            if (abiItem.outputs.length === 1) {
                ...
                if (helpers_1.default.isNeverModifyBlockchainState(abiItem)) {
                    var interfaceName = helpers_2.default.buildResponseInterfaceName(abiItem);
                    var ouputProperties = '';
